...
class ApiAiJoinTest extends Test 
{
	...
	public function testJoin(async:Async) {
		...
		Test.api.aiJoin("1234", function(game:ExternalGameRecordObject, error:ErrorObject) {
			Assert.isNull(error);
			...
			Assert.equals(Handlers.AI_PLAYER, Reflect.field(game, 'player'));
